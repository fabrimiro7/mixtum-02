...
# Project root (deploy.sh lives in scripts/)
...
if [[ $DRY_RUN_RC -ne 0 ]]; then
  ...
  warn "  docker compose -f docker/docker-compose.yml -f docker/docker-compose.prod.yml logs --tail=200 certbot"
else
